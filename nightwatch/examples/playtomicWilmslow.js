...
describe('playtomic example', function () {
	it('Search Nightwatch.js and check results', (browser) => {
		browser
			.navigateTo(`https://playtomic.io/the-padel-club-wilmslow/c1b6a3e4-3528-42fd-8b24-4b5ecfec947c?q=PADEL~${day.format("YYYY-MM-DD")}~~~`)
			...
			.execute(() => {
				...
				fs.appendFileSync(path.join(__dirname , '../../data/playtomic_wilmslow.tsv'), csv);
			});
	});
});